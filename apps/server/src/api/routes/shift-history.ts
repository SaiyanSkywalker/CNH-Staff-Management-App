...
shiftHistoryRouter.get("/", async (req, res) => {
  ...
  const { employeeId, employeeName, unit, requestedDate, shiftDate, shift, status } = req.query;
  ...
  if(requestedDate) {
    ...
    attributes.createdAt = parsedRequestedDate.substring(5, 7) + "/" + parsedRequestedDate.substring(8, 10) + "/" + parsedRequestedDate.substring(0, 4);
    console.log(`attributes.createdAt is: ${attributes.createdAt}`);
  }
  if(shiftDate) {
    const parsedDate: string = shiftDate as string;
    attributes.dateRequested = parsedDate.substring(5,7) + "/" + parsedDate.substring(8, 10) + "/" + parsedDate.substring(0, 4);
  }
  if(shift) {
