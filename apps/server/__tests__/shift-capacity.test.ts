...
describe('POST /shift-capacity', () => {
    ...
    it('post request with incorrect body types', async () => {
        ...
        expect(response.body.msg).toStrictEqual("shiftTime property in request's body should be a string and capacities should be an object of keys of type strings and values of type number");
    });
    ...
    describe('Posting a shift capacity', () => {
        it('post a non-existent shift capacity', async () => {
            ...
            expect(response.status).toBe(200);
            expect(response.body.updatedShifts).toStrictEqual([
                {id: 1, unitId: 3, shift: '03:00 - 07:00' , capacity: 30, shiftDate: '2024-05-28'},
            ])
        });
        ...
        it('post an existing shift capacity', async () => {
            ...
            expect(response.status).toBe(200);
            expect(response.body.updatedShifts).toStrictEqual([
                {id: 2, unitId: 1, shift: '03:00 - 07:00' , capacity: 40, shiftDate: '2024-05-28'},
            ])
        });
        ...
        it('post shift capacity for multiple existing units', async () => {
            ...
            expect(response.body.updatedShifts).toStrictEqual([
                {id: 2, unitId: 1, shift: '03:00 - 07:00' , capacity: 40, shiftDate: '2024-05-28'},
                {id: 4, unitId: 2, shift: '03:00 - 07:00' , capacity: 40, shiftDate: '2024-05-28'},
            ])
        });
    });
    ...
    it('post request with no corresponding id', async () => {
        ...
        .send(shiftCapacity);
        expect(response.status).toBe(400);
        expect(response.body.msg).toStrictEqual("Need a shift time and capacity configured for at least one unit!");
    });
    ...
    it('post shift where next day is beginning of next month', async () => {
        ...
        .send(shiftCapacity);
        expect(response.status).toBe(200);
        expect(response.body.updatedShifts[0].shiftDate).toStrictEqual('2024-04-30');
        expect(response.body.updatedShifts[1].shiftDate).toStrictEqual('2024-05-01');
    });
    ...
    it('post shift where next day is beginning of next year', async () => {
        ...
        .send(shiftCapacity);
        expect(response.status).toBe(200);
        expect(response.body.updatedShifts[0].shiftDate).toStrictEqual('2024-12-31');
        expect(response.body.updatedShifts[1].shiftDate).toStrictEqual('2025-01-01');
    });
});
...
describe('GET /shift-capacity/mobile', () => {
    ...
    it('get a shift capacity from mobile site without costCenterId', async () => {
        const response = await request(app)
        .get('/shift-capacity/mobile?date=2024-05-28');
        expect(response.status).toEqual(400);
        expect(response.body.msg).toEqual("costCenterId should be a valid integer, and date should be a string in format: 'YYYY-MM-DD'");
    });

    it('get a shift capacity from mobile site without date', async () => {
        const response = await request(app)
        .get('/shift-capacity/mobile?costCenterId=1');
        expect(response.status).toEqual(400);
        expect(response.body.msg).toEqual("costCenterId should be a valid integer, and date should be a string in format: 'YYYY-MM-DD'");
    });
    ...
    it('get a shift capacity from mobile site with valid query parameters', async () => {
        const response = await request(app)
        .get('/shift-capacity/mobile?costCenterId=1&date=2024-05-28');
        expect(response.status).toEqual(200);
    });
});
