...
export const DefaultCapacityMock = {
    ...
    findOne: jest.fn((query): Promise<DefaultCapacityAttributes | null> => {
        ...
        else if(query['where']['shift'] === '15:00 - 19:00' && query['where']['unitId'] == 1) {
            return Promise.resolve(defaultCapacityOne);
        }
        else if(query['where']['shift'] === '19:00 - 23:00' && query['where']['unitId'] == 1) {
            return Promise.resolve(defaultCapacityTwo);
        }
        else if(query['where']['shift'] === '23:00 - 03:00' && query['where']['unitId'] == 1) {
            return Promise.resolve(defaultCapacityThree);
        }
        else if(query['where']['shift'] === '03:00 - 07:00' && query['where']['unitId'] == 1) {
    ...
    create: jest.fn((query): Promise<DefaultCapacityAttributes | null> => {
        let defaultCapacity: DefaultCapacityAttributes = {id: 1, unitId: query['unitId'], capacity: query['capacity'], shift: query['shift'] }
        return Promise.resolve(defaultCapacity);
    }),
    findAll: jest.fn((query) => {
        const unit1: UnitAttributes = { id: 1, laborLevelEntryId: 1, name: "ECMO", description: "" };
        const unit2: UnitAttributes = { id: 2, laborLevelEntryId: 3, name: "NICU", description: "" };

        let defaultCapacityOne = {id: 1, unitId: 1, unit: unit1, shift: '23:00 - 03:00' , capacity: 40};
        let defaultCapacityTwo = {id: 2, unitId: 1, unit: unit1, shift: '03:00 - 07:00' , capacity: 40};
        let defaultCapacityFull = {id: 3, unitId: 1, unit: unit1, shift: '23:00 - 07:00' , capacity: 40};
        let secondUnitDefaultCapacity = {id: 4, unitId: 2, unit: unit2, shift: '03:00 - 07:00' , capacity: 30};
                
        if(!query || !query['include'] || (query['include'].length === 0) || !query['include'][0]['where'] || Object.keys(!query['include'][0]['where']).length === 0 || !query['include'][0]['where']['id']) {
            return Promise.resolve([defaultCapacityOne, defaultCapacityTwo, defaultCapacityFull, secondUnitDefaultCapacity]);
        }

        let id: number = query['include'][0]['where']['id'];
        if (id === 1) {
            return Promise.resolve([defaultCapacityOne, defaultCapacityTwo, defaultCapacityFull]);
        }
        else if (id === 2) {
...
export const ShiftCapacityMock = {
    findOne: jest.fn((query): Promise<ShiftCapacityAttributes | null> => {
        ...
        else if(query['where']['shift'] === shiftCapacityTwo.shift && query['where']['unitId'] == shiftCapacityTwo.unitId && query['where']['shiftDate'] === shiftCapacityTwo.shiftDate) {
            return Promise.resolve(shiftCapacityTwo);
        }
        else if(query['where']['shift'] === shiftCapacityFull.shift && query['where']['unitId'] == shiftCapacityFull.unitId && query['where']['shiftDate'] === shiftCapacityFull.shiftDate) {
            return Promise.resolve(shiftCapacityFull);
        }
        else if(query['where']['shift'] === secondUnitShiftCapacity.shift && query['where']['unitId'] == secondUnitShiftCapacity.unitId && query['where']['shiftDate'] === secondUnitShiftCapacity.shiftDate) {
    ...
    create: jest.fn((query): Promise<ShiftCapacityAttributes | null> => {
        let shiftCapacity: ShiftCapacityAttributes = {id: 1, unitId: query['unitId'], capacity: query['capacity'], shift: query['shift'], shiftDate: query['shiftDate'] }
    ...
    findAll: jest.fn((query) => {
        ...
        let shiftCapacityOne = {id: 1, unitId: 1, unit: unit1, shift: '23:00 - 03:00' , capacity: 40, shiftDate: '2024-05-28'}
        let shiftCapacityTwo = {id: 2, unitId: 1, unit: unit1, shift: '03:00 - 07:00' , capacity: 40, shiftDate: '2024-05-28'}
        let shiftCapacityFull = {id: 3, unitId: 1, unit: unit1, shift: '23:00 - 07:00' , capacity: 40, shiftDate: '2024-05-28'}
        let secondUnitShiftCapacity = {id: 4, unitId: 2, unit: unit2, shift: '03:00 - 07:00' , capacity: 30, shiftDate: '2024-05-28'}
                
        if(!query || !query['include'] || (query['include'].length === 0) || !query['include'][0]['where'] || Object.keys(!query['include'][0]['where']).length === 0 || !query['include'][0]['where']['id'] || Object.keys(!query['where']).length === 0 || !query['where']['shiftDate']) {
            return Promise.resolve([shiftCapacityOne, shiftCapacityTwo, shiftCapacityFull, secondUnitShiftCapacity]);
        }

        let id: number = query['include'][0]['where']['id'];
        let date: string = query['where']['shiftDate'];
        if (id === 1 && date === '2024-05-28') {
            return Promise.resolve([shiftCapacityOne, shiftCapacityTwo, shiftCapacityFull]);
        }
...
export const ShiftHistoryMock = {
    findOne: jest.fn(),
    findAll: jest.fn((query) => {
        const user1: UserInformationAttributes = { employeeId: 1, username: "firstUser", firstName: "First", lastName: "User", password: "1234" }
        const user2: UserInformationAttributes = { employeeId: 2, username: "secondUser", firstName: "Second", lastName: "User", password: "1234" }
        const user3: UserInformationAttributes = { employeeId: 3, username: "thirdUser", firstName: "Third", lastName: "User", password: "1234" }
        const user4: UserInformationAttributes = { employeeId: 4, username: "fourthUser", firstName: "Fourth", lastName: "User", password: "1234" }

        const unit1: UnitAttributes = { laborLevelEntryId: 1, name: "ECMO", description: "" };
        const unit3: UnitAttributes = { laborLevelEntryId: 3, name: "NICU", description: "" };

        const shiftHistory1 = { id: 1, shiftTime: "07:00 - 11:00", status: "Accepted", user: user1, unit:unit1, userId: 1, unitId: 1, dateRequested: "04/30/2024", createdAt: new Date(2024, 3, 30) };
        const shiftHistory2 = { id: 2, shiftTime: "07:00 - 11:00", status: "Rejected", user: user2, unit:unit1, userId: 2, unitId: 1, dateRequested: "05/10/2024", createdAt: new Date(2024, 4, 2) };
        const shiftHistory3 = { id: 3, shiftTime: "15:00 - 19:00", status: "Accepted", user: user3, unit:unit3, userId: 3, unitId: 3, dateRequested: "05/11/2024", createdAt: new Date(2024, 4, 3) };
        const shiftHistory4 = { id: 4, shiftTime: "15:00 - 19:00", status: "Pending", user: user1, unit:unit1, userId: 1, unitId: 1, dateRequested: "05/11/2024", createdAt: new Date(2024, 4, 3) };
        const shiftHistory5 = { id: 5, shiftTime: "19:00 - 23:00", status: "Accepted", user: user3, unit:unit3, userId: 3, unitId: 3, dateRequested: "05/11/2024", createdAt: new Date(2024, 4, 3) };
        const shiftHistory6 = { id: 6, shiftTime: "19:00 - 23:00", status: "Rejected", user: user2, unit:unit1, userId: 2, unitId: 1, dateRequested: "05/11/2024", createdAt: new Date(2024, 4, 3) };
        const shiftHistory7 = { id: 7, shiftTime: "23:00 - 03:00", status: "Accepted", user: user3, unit:unit3, userId: 3, unitId: 3, dateRequested: "05/12/2024", createdAt: new Date(2024, 4, 4) };
        const shiftHistory8 = { id: 8, shiftTime: "23:00 - 03:00", status: "Accepted", user: user4, unit:unit3, userId: 4, unitId: 3, dateRequested: "05/12/2024", createdAt: new Date(2024, 4, 4) };
        const shiftHistory9 = { id: 9, shiftTime: "15:00 - 19:00", status: "Pending", user: user4, unit:unit3, userId: 4, unitId: 3, dateRequested: "05/12/2024", createdAt: new Date(2024, 4, 5) };
        let shiftHistories = [shiftHistory1, shiftHistory2, shiftHistory3, shiftHistory4, shiftHistory5, shiftHistory6, shiftHistory7, shiftHistory8, shiftHistory9]

        if(query['where']['shiftTime']) {
            shiftHistories = shiftHistories.filter(shiftHistory => shiftHistory['shiftTime'] === query['where']['shiftTime']);
        }

        if(query['where']['status']) {
            shiftHistories = shiftHistories.filter(shiftHistory => shiftHistory['status'] === query['where']['status']);
        }

        if(query['where']['userId']) {
            shiftHistories = shiftHistories.filter(shiftHistory => shiftHistory['userId'] == query['where']['userId']);
        }

        if(query['where']['unitId']) {
            shiftHistories = shiftHistories.filter(shiftHistory => shiftHistory['unitId'] == query['where']['unitId']);
        }

        if(query['where']['dateRequested']) {
            shiftHistories = shiftHistories.filter(shiftHistory => shiftHistory['dateRequested'] == query['where']['dateRequested']);
        }
